...
export const Index: Record<string, any> = {`
  for (const item of registry.items) {
    const resolveFiles = item.files?.map(
      (file) => `registry/classic/${file.path}`
    )
    if (!resolveFiles) {
      continue
    }
  ...
  "${item.name}": {
    ...
    files: ${JSON.stringify(filesWithContent)},
...
async function buildRegistryJsonFile() {
  // 1. Add registry/classic prefix for the build to work
  const fixedRegistry = {
    ...registry,
    items: registry.items.map((item) => {
      const files = item.files?.map((file) => {
        return {
          ...file,
          path: `registry/classic/${file.path}`,
        }
      })
  ...
  // 3. Copy the registry.json to the www/dist/client/r/styles/classic directory.
  await fs.cp(
    path.join(process.cwd(), "registry.json"),
    path.join(
      process.cwd(),
      "../www/dist/client/r/styles/classic/registry.json"
    ),
    { recursive: true }
  )
}
...
async function buildRegistry() {
  return new Promise((resolve, reject) => {
    exec(
      `pnpm dlx shadcn build registry.json --output dist/client/r`,
      async (error, stdout, stderr) => {
        ...
        // Post-process the generated JSON files to remove registry/classic prefix
...
async function cleanupGeneratedPaths() {
  const publicDir = path.join(process.cwd(), "dist/client/r")
    ...
    if (json.files) {
      ...
      json.files.forEach((file: any) => {
        if (typeof file === "object" && file.path && file.target) {
          ...
          const sourcePath = file.path.replace(/^registry\/classic\//, "")
      ...
      json.files = json.files.map((file: any) => {
        if (typeof file === "object" && file.path) {
          // Remove registry/classic/ prefix
          let cleanPath = file.path.replace(/^registry\/classic\//, "")
...
function rewriteImports(
  ...
  // Rewrite imports from @/registry/classic/ui/... to @/components/ui/...
  content = content.replace(
    /@\/registry\/classic\/ui\//g,
    "@/components/ui/"
  )

  // Rewrite imports from @/registry/classic/examples/... to @/components/examples/...
  content = content.replace(
    /@\/registry\/classic\/examples\//g,
    "@/components/examples/"
  )

  // Rewrite imports from @/registry/classic/lib/... to @/lib/...
  content = content.replace(/@\/registry\/classic\/lib\//g, "@/lib/")
...
async function buildAllJson() {
  ...
  for (const component of uiComponents) {
    ...
    if (component.files) {
      for (const file of component.files) {
        const filePath = `registry/classic/${file.path}`
        ...
        // Clean path - remove registry/classic/ prefix
  ...
  // Write to dist/client/r/all.json
  const outputPath = path.join(process.cwd(), "dist/client/r/all.json")
